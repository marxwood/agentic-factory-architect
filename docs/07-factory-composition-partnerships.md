...
In practice, this is often not the case. MyCompany may have data validation capabilities but not the SAP integration expertise that Company X's commission requires. Agent Y — a systems integrator — has exactly that expertise.
...
Company X sees one executor: MyCompany. Company X does not need to know that Agent Y exists. MyCompany is responsible for the entire delivery — including the part Agent Y produces.
...
- Agent Y contributes capability that MyCompany genuinely lacks

The relationship is collaborative, not subordinate. But within the scope of the commission, MyCompany holds **primary responsibility** — because that's what Company X contracted for.

### The Three-Layer Trust Chain

```
Company X trusts MyCompany (dual-origin commission)
    MyCompany trusts Agent Y (partnership agreement + derived commission)
        Agent Y's factory executes the sub-commission
```

Trust does not transitively flow from Company X to Agent Y. Company X trusts MyCompany. MyCompany trusts Agent Y. These are separate trust relationships with separate terms.

This means:
- Company X's constraints flow through MyCompany to Agent Y — but MyCompany is responsible for enforcing them
- Agent Y's output goes to MyCompany, not to Company X — MyCompany validates before assembly
- If Agent Y violates a constraint, MyCompany is accountable to Company X — MyCompany's relationship with Agent Y is MyCompany's problem to manage

### Derived Commission

When MyCompany's Decomposition Engine determines that WU-3 (SAP integration) requires capability it doesn't have, it creates a **derived commission** for Agent Y.
...
- **Adds MyCompany's own constraints** — MyCompany may impose additional constraints on Agent Y (delivery format, timeline, interface specifications) that come from MyCompany's MB-OS, not Company X's
- **Is immutable** — same rule as any commission

```yaml
derived_commission:
  id: "dc-mycompany-agenty-sap-001"
  type: DERIVED
  parent_commission: "c-inter-001"  # reference only, not full content

  derivation:
    derived_by: "MyCompany"
    work_unit_reference: "WU-3"
    reason: "SAP integration expertise not available in MyCompany's agent registry"
  ...
  commissioner:
    ...
    constraints:
      hard:
        - "Output must conform to MyCompany's integration interface spec v2.1"
        - "All data handling must comply with EU jurisdiction (inherited from parent)"
        - "No direct contact with Company X — all communication through MyCompany"
      soft:
  ...
  shared:
    entities:
      - name: "SAP Integration Layer"
        commissioner_definition: "The component that connects MyCompany's validation output to Company X's SAP environment"
        executor_definition: "A middleware layer using certified SAP connectors to ingest validated data"
        agreed_definition: "A middleware component that receives MyCompany-validated data and writes it to SAP using certified BAPI connectors"

    validity_criteria:
      - criterion: "Integration handles all data types specified in MyCompany's output schema"
        evaluator: "MyCompany's technical validation gate"
...
The most critical architectural element in a partnership is the **composition boundary** — the validation gate that sits between Agent Y's output and MyCompany's assembly layer.

This gate is MyCompany's responsibility. It checks:

1. **Contract fulfillment** — did Agent Y deliver what the derived commission specified?
2. **Interface compliance** — does the output match MyCompany's expected integration format?
3. **Constraint inheritance** — were the constraints inherited from Company X's commission preserved?
4. **Entity consistency** — does Agent Y's output use entities consistently with how MyCompany defined them?
5. **No scope leakage** — did Agent Y's work stay within the boundaries of the derived commission?

```
Agent Y's Factory
    ↓ delivers
[Composition Boundary Gate]     ← MyCompany-owned, binary validity
    ↓ pass
MyCompany's Assembly Layer
...
If Agent Y's output fails the composition boundary gate, it's returned to Agent Y — not escalated to Company X. MyCompany manages the relationship. Company X sees only the final delivery.

## Partnership Negotiation Flow

### Phase 1: Capability Gap Detection

During decomposition, MyCompany's factory identifies a work unit that no agent in its registry can fulfill.
...
Decomposition Engine → MyCompany's MB-OS: "Commission requires capability we don't have.
  Options: (a) decline commission, (b) find a partner, (c) escalate to Company X"
```

### Phase 2: Partner Discovery

MyCompany's MB-OS evaluates potential partners against its own values and constraints:
...
- Does partnership with Agent Y conflict with any of MyCompany's values?

### Phase 3: Partnership Negotiation

MyCompany's agent and Agent Y's agent negotiate using the Inter-MB-OS Protocol — the same protocol used between Company X and MyCompany, now applied one level down.

The negotiation produces a derived commission with:
- Clear scope (only the capability gap, not the whole commission)
- Combined constraints (inherited + MyCompany's own + Agent Y's own)
- Interface specification (how Agent Y's output integrates with MyCompany's factory)
...
This updated work graph may be shared with Company X during the review phase (Phase 7 of the Inter-MB-OS Protocol) — with or without revealing Agent Y's identity, depending on MyCompany's transparency preferences and Company X's requirements.

### Phase 5: Parallel Execution

Agent Y's factory executes the derived commission in parallel with MyCompany's own work units. Both factories operate independently — Agent Y's orchestration layer is separate from MyCompany's.

### Phase 6: Composition

Agent Y delivers. MyCompany's composition boundary gate validates. If it passes, the output flows into MyCompany's assembly layer alongside the outputs of MyCompany's own work units.
...
Each partner receives its own derived commission. Each has a composition boundary gate. MyCompany's assembly layer integrates all outputs.
...
Dual-Origin Commission (X ↔ MyCompany)
    ↓ source of truth for: the work
MyCompany's MB-OS
    ↓ source of truth for: execution approach
Derived Commission (MyCompany ↔ Agent Y)
...
If Agent Y discovers that the derived commission is inadequate — that the SAP integration requires something MyCompany didn't specify — Agent Y surfaces the gap to MyCompany. MyCompany decides whether to update the derived commission (issuing a new one) or escalate to Company X if it affects the original commission.
...
| Fulfilling the full commission | MyCompany |
| Selecting trustworthy partners | MyCompany |
| Validating partner output | MyCompany (composition boundary gate) |
| Delivering the derived commission | Agent Y |
| Agent Y's internal quality | Agent Y |
| Final assembly and delivery | MyCompany |
| Evaluating "is this mine?" | Company X |

**Critical rule:** MyCompany cannot deflect responsibility to Agent Y when delivering to Company X. "Agent Y didn't deliver on time" is MyCompany's problem, not Company X's. This is what makes partnership different from referral.

### Skill 9 — Explicit Change Control

When Agent Y updates its factory (new model, new SAP connector version), this is a change that affects MyCompany's composition. The change control protocol applies:

1. Agent Y declares the change to MyCompany
2. MyCompany evaluates impact on the derived commission and composition boundary
...
### Interface Mismatch
Agent Y builds to their understanding of the interface spec. MyCompany expects something slightly different. The composition boundary gate catches this — but only if the interface spec in the derived commission was precise enough. Vague interface specifications are the most common source of composition failure.

### Trust Chain Erosion
Over time, partnerships can drift. Agent Y may quietly change their approach, use different subcontractors, or relax their quality standards. Without explicit change control at the partnership boundary, MyCompany may not notice until delivery fails. The governance model requires: partnership agreements are versioned artifacts, subject to the same change control as factory configurations.
...
### Depth Creep
Agent Y partners with Agent Z2. Agent Z2 partners with Agent Z3. Before long, the original commission is being executed five levels deep by agents that Company X has never heard of and MyCompany can barely audit. The governance model must enforce: maximum composition depth, declared at the original commission level.
...
The work graph remains the negotiation artifact at every boundary. Whether it's your MB-OS commissioning your own factory, Company X commissioning MyCompany, or MyCompany commissioning Agent Y — the pattern is the same: **structured intent, explicit constraints, reviewable plan, validated execution, traceable delivery**.
